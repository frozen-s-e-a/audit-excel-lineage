...
**当前状态：0.2 版方案与数据契约已准备，提取器和桌面程序尚未实现。** 当前技术路线为 **C# / .NET 10 LTS + WPF + Open XML SDK + SQLite**。

已知实际规模：用户目前最大的 Excel 文件约 **50MB**，文件后缀、实际单元格/公式数及办公电脑配置尚未确认。采用流式读取、受限并发、区域依赖和分页展示，并把该文件作为后续本地验收样本；当前没有实际性能测试结果。
